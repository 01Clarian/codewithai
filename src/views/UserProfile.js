  ...
  const updateStripeEmail = async (updatedEmail) => {
    ...
    try {
      ...
      if (customer) {
        ...
        await fetch('https://api.codewithai.org/update-stripe-email', {
          method: 'POST',
          headers: {
  ...
  useEffect(() => {
    ...
    const checkSubscriptionStatus = async () => {
      ...
      try {
        ...
        const subResponse = await fetch(`https://api.codewithai.org/get-subscription-id?customerId=${customer.id}`);
...
const reauthenticateUser = async (user) => {
  ...
  const handleDeleteAccount = async (e) => {
    ...
    if (confirmed) {
      ...
      try {
        // Retrieve the customer object from the backend API using the email address
        const response = await fetch(`https://api.codewithai.org/${user.email}`);
        const data = await response.json();
        const customer = data.customer;
        ...
        if (subscriptionId) {
          ...
          const cancelResponse = await fetch('https://api.codewithai.org/cancel-subscription', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
  ...
  const handleCheckoutClick = async (event) => {
    ...
    const response = await fetch('https://api.codewithai.org/create-checkout-session', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
